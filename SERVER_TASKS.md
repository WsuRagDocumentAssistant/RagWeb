...
| **`CHAT_SESSION_LIST`** | RAG | **신규** | 좌측 사이드바 대화 목록 조회 (제목만) |
| **`CHAT_SESSION_MESSAGES`** | RAG | **신규** | 사이드바에서 대화 클릭 시 메시지 내역 조회 |
| **`CHAT_SESSION_DELETE`** | RAG | **신규** | 좌측 사이드바 대화 삭제 버튼 |
...
### `CHAT_SESSION_LIST` (신규)
로그인 직후 좌측 사이드바의 대화 목록을 채우기 위해 호출됩니다. 지금까지는 대화가 전부 브라우저
`localStorage`에만 저장되어 있어서, 기기를 바꾸거나 캐시를 지우면 대화 기록이 전부 사라졌습니다.

**목록에는 메시지 내역을 포함하지 마세요.** 대화가 많고 각 대화의 메시지도 많을 수 있어서, 목록
조회 한 번에 전체 내역을 다 실어 보내면 무거워집니다. 제목/메타데이터만 내려주면, 클라이언트가
사이드바에서 어떤 대화를 클릭했을 때만 `CHAT_SESSION_MESSAGES`로 그 대화의 내역을 따로 가져갑니다
(화면에 바로 보이는 첫 번째 대화만 목록 직후 자동으로 한 번 더 조회합니다).
```json
// 요청 payload: 없음 (Authorization 토큰으로 사용자 식별)
// 응답 result
{
  "sessions": [
    { "sessionId": "...", "title": "회의록 관련 질문", "createdAt": 1735600000000 }
  ]
}
```
서버 응답이 비어 있는 배열이면 클라이언트는 기존 로컬 대화 목록을 그대로 유지합니다(최초 가입 등
서버에 대화가 하나도 없는 경우 대비).

### `CHAT_SESSION_MESSAGES` (신규)
사이드바에서 특정 대화를 클릭했을 때 호출됩니다. 이미 한 번 불러온 대화는 같은 세션 동안 다시
요청하지 않고 캐시된 내역을 그대로 씁니다.
```json
// 요청 payload
{ "sessionId": "..." }
// 응답 result
  ...
  "messages": [
    {
      ...
      "provider": "claude | gemini | gpt | local | merged",  // role이 assistant일 때만
      "turnId": "...",           // 같은 질문에 대한 여러 모델 답변을 묶는 값
      "preferred": true,          // 선택, 비교 답변 중 사용자가 고른 것
      "mergerProvider": "gpt",    // 선택, provider가 merged일 때 병합을 수행한 모델
      "sources": [{ "id": "...", "name": "..." }]  // 선택
    }
  ]
}
```
호출이 실패하면 해당 대화는 빈 채로 남고, 사용자가 다시 클릭하면 재시도합니다.

### `CHAT_SESSION_DELETE` (신규)
사이드바의 대화 삭제(휴지통 아이콘) 버튼에서 호출됩니다.
```json
// 요청 payload
{ "sessionId": "..." }
// 응답 result — 클라이언트가 필드를 읽지 않음 (낙관적으로 이미 화면에서 삭제됨)
{}
```
