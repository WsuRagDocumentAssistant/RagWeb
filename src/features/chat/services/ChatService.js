 ...
 * @param {{ query: string, answers: { provider: string, content: string, sources?: { id: string, name: string }[] }[], provider: string, sessionId?: string }} payload provider는 병합 작업을 수행할 모델
 * @returns {Promise<{ reply: string, sources?: { id: string, name: string }[] }>}
 */
export async function mergeResults({ query, answers, provider, sessionId }) {
